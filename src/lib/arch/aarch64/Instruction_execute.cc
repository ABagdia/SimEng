    ...
    switch (metadata.opcode) {
      ...
      case Opcode::AArch64_CMPEQ_PPzZI_D: {  // cmpeq pd.d, pg/z, zn.d, #imm
        ...
        for (int i = 0; i < partition_num; i++) {
          ...
          if (p[i / 8] & shifted_active) {
            out[i / 8] =
                (n[i] == m) ? (out[i / 8] | shifted_active) : out[i / 8];
          }
        }
      ...
      case Opcode::AArch64_FCSELDrrr: {  // fcsel dd, dn, dm, cond
        ...
        results[0] = RegisterValue(
            conditionHolds(metadata.cc, operands[0].get<uint8_t>()) ? n : m,
            256);
        break;
      }
      ...
      case Opcode::AArch64_MOVZWi: {  // movz wd, #imm
        uint8_t shift = metadata.operands[1].shift.value;
        uint32_t value = static_cast<uint64_t>(metadata.operands[1].imm)
                         << shift;
        results[0] = RegisterValue(value, 8);
        break;
      }
      case Opcode::AArch64_MOVZXi: {  // movz xd, #imm
        uint8_t shift = metadata.operands[1].shift.value;
        uint64_t value = static_cast<uint64_t>(metadata.operands[1].imm)
                         << shift;
        results[0] = value;
        break;
      }
      ...
      case Opcode::AArch64_SBFMWri: {  // sbfm wd, wn, #immr, #imms
        ...
        results[0] =
            RegisterValue(bitfieldManipulate(source, 0u, r, s, true), 8);
        break;
      }
      ...
      case Opcode::AArch64_UCVTFUWDri: {  // ucvtf dd, wn
        results[0] = RegisterValue(
            static_cast<double>(operands[0].get<uint32_t>()), 256);
        break;
      }
      ...
      case Opcode::AArch64_UCVTFUXDri: {  // ucvtf dd, xn
        results[0] = RegisterValue(
            static_cast<double>(operands[0].get<uint64_t>()), 256);
        break;
      }
